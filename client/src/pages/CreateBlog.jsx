...
function CreateBlog() {
  ...
  const handleUpdateBlock = (id, updatedData) => {
    setBlocks(
      blocks.map((block) =>
        block.id === id ? { ...block, data: { ...block.data, ...updatedData } } : block
      )
    );
  };
  ...
  // Upload block images to Cloudinary to save URLs inside JSON blocks
    ...
    for (const block of blocks) {
      if (block.type === "image" && block.data.file) {
        ...
        try {
          ...
          formData.append("image", block.data.file); // Reuse uploads system
          // Note: using direct travel memory upload API or other upload routers
          // Since we need to upload files, let's create a generic upload endpoint or use trip uploads.
          // Wait, is there a generic upload endpoint? Let's check profile.routes.js or memory.routes.js.
          // In TripShare, files are usually uploaded via upload middleware.
          // If we upload images, we can do it inline or we can serialize files into FormData at post time.
          // Yes! To avoid complicating uploads, we can append all block files directly to the main create request,
          // OR we can use standard Image URL input, OR we can upload them during submit.
          // Let's implement block file attachment by indexing them in FormData, e.g. "block_image_<id>".
          // The backend can then upload them all, and replace the block url with the Cloudinary URL!
          ...
          console.error("Failed preparing image block:", err);
        }
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If blocks have image files, upload them as well
  ...
  return (
    <div className="container py-5 mt-4" style={{ maxWidth: "800px" }}>
      <h2 className="text-warning fw-bold mb-4">Create Travel Story 📝</h2>
      ...
      <form onSubmit={handleSubmit} className="d-flex flex-column gap-4">
          ...
          >
            {coverPreview ? (
              <div className="position-relative">
                <img src={coverPreview} alt="Cover Preview" className="img-fluid rounded" style={{ maxHeight: "200px", objectFit: "cover", width: "100%" }} />
                <button 
                  type="button" 
        ...
        <div className="glass-card p-4 d-flex flex-column gap-3">
          <div>
            ...
            <input
              ...
              placeholder="Give your travel story a stunning name..."
              value={title}
        ...
        <div className="glass-card p-4">
          ...
          <div className="d-flex flex-wrap gap-2 mb-3">
            ...
            {tags.length === 0 && (
              <span className="text-secondary small italic py-1">No tags added yet. Add tags for traveler searches.</span>
            )}
          </div>
